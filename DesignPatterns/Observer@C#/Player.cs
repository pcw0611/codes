...
class Player 
{
	static void Main( string[] args )
	{
		...
		// 이러한 1:n의 관계로서 객체 간 느슨한 결합을 통해 상태의 변화를 통보하는 것이 옵저버 패턴이다.
		...
		Inventory			inventory	= new Inventory();			// Subject
		CharacterModel		model		= new CharacterModel();		// Observerable
		ChartacterStatus	status		= new ChartacterStatus();	// Observerable

		// Sbuscrible 
		inventory.AddEquipObserverable( model );
		inventory.AddEquipObserverable( status );

		// 만약 추후 채팅창에 'X 아이템을 장착하였습니다.' 라는 메세지를 추가 해달라는 기획 요청 건이 들어왔다.
		// ChatArea 클래스는 OnEquip 인터페이스를 가지고, 인벤토리에서는 아래와 같은 리스너를 추가해주기만 하면된다.
		// ChatArea에서는 인벤토리 자체를 참조할 필요가 없으며, 코드와 구조가 훨씬 깔끔해진다. 단순히 '장착을 감지할 수 있는 ChatArea'의 역할로서
		// 자기주도적인 클래스가 되는 것이다.
		// inventory.AddEquipObserverable( chatArea );
